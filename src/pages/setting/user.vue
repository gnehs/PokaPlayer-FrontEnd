      ...
      <div class="item" v-ripple @click="setDataRecord">
        <div class="content">
          ...
          <div class="header">
            ...
            <div
              class="t-ellipsis"
            >{{$t('settingPravicy.dataRecord.'+(dataRecord?'enabled':'disabled'))}}</div>
          </div>
        </div>
      </div>
    ...
    <v-dialog v-model="temp.changeNamePrompt" max-width="300">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            text
            @click="temp.changeNamePrompt=false;changeName()"
            color="primary"
          >{{$t('done')}}</v-btn>
        </v-card-actions>
      </v-card>
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            color="primary"
          >{{$t('settingUser.changePassword.done')}}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  async created() {
    //使用者
    this.userdata = (
      await this.axios.get(_setting(`server`) + "/pokaapi/v2/user/")
    ).data;
    //隱私
    this.dataRecordCount = (
      await this.axios(
        _setting(`server`) + `/pokaapi/v2/record/count/user?${Date.now()}`
      )
    ).data;
  },
  methods: {
    ...
    changeUsername() {
      if (this.temp.changeUsernameValue == "" || !this.temp.changeUsernameValue)
        return this.$snackbar(
          this.$t("settingUser.changeUsername.result.error")
        );
      this.axios
    ...
    changePassword() {
      if (
        ...
        return this.$snackbar(
          this.$t("settingUser.changePassword.result.error")
        );
      if (this.temp.changePassword !== this.temp.changePassword2)
        return this.$snackbar(
          this.$t("settingUser.changePassword.result.inconsistent")
        );
      if (this.temp.changePassword === this.temp.changePasswordold)
        return this.$snackbar(
          this.$t("settingUser.changePassword.result.same")
        );
      this.axios
        ...
        .then(response => {
          if (response.data.success) {
            this.$snackbar(
              this.$t("settingUser.changePassword.result.success")
            );
    ...
    logout() {
      ...
      _player.pause();
      this.axios(_setting(`server`) + "/logout").then(e =>
    ...
    async clearRecord() {
      if (confirm("您確定要清除資料嗎？"))
        this.axios
          .post(_setting(`server`) + `/pokaapi/v2/record/clear`)
          .then(r => {
            this.$snackbar("資料已清除");
