...
export default {
  ...
  methods: {
    ...
    async createUser() {
      ...
      let createUserResult = await this.axios.post(
        _setting(`server`) + "/pokaapi/v2/users/create",
      ...
      if (createUserResult.data.success) {
        ...
        if (createUserResult.data.error == "username already taken") {
          this.$snackbar(
            i18n.t("settingUserManagement.dialog.userCreateUsernameRepeat")
          );
        } else {
          this.$snackbar(
            i18n.t("settingUserManagement.dialog.userCreateFailed")
          );
        }
      }
    },
    async changePassword() {
      let password = prompt(
        i18n.t("settingUserManagement.dialog.enterNewPassword")
      );
      if (password) {
        await this.axios
          ...
          .catch(e => {
            this.$snackbar(
              i18n.t("settingUserManagement.dialog.changePasswordFailed")
            );
          });
        this.$snackbar(
          i18n.t("settingUserManagement.dialog.changePasswordSuccess")
        );
      }
    },
    async deleteUser() {
      if (confirm(`Are you sure want to delete "${this.userTemp.name}"?`)) {
        await this.axios
          ...
          .catch(e => {
            this.$snackbar(
              i18n.t("settingUserManagement.dialog.deleteUserFailed")
            );
          });
