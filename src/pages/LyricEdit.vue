  ...
  methods: {
    loadLrc(lrc) {
      ...
      if (
        ...
        window._lrc.lyrics_all[window._lrc.lyrics_all.length - 2].text =
          window._lrc.lyrics_all[window._lrc.lyrics_all.length - 1].text;
    ...
    timeShift(val) {
      ...
      for (let i in lyric_temp) {
        lyric_temp[i].timestamp = this.minus(
          lyric_temp[i].timestamp,
          this.times(val, -1)
        );
        lrc_temp += `[${this.timeToTag(lyric_temp[i].timestamp)}]${
          lyric_temp[i].text
        }\n`;
      }
      this.lyric_raw = lrc_temp;
      this.timeShiftValue = this.minus(
        this.timeShiftValue,
        this.times(val, -1)
      );
    },
          ...
          this.$nextTick(() => {
            ...
            if (focusedLyric) {
              let sh =
                ...
                document.querySelector(".lyric-view").clientHeight * 0.5 +
                focusedLyric.clientHeight *
                  (this.lyricTranslated ? 1.75 : 0.75);
