...
export default {
  ...
  methods: {
            ...
            if (this.lyricFocus != lyricFocus_temp) {
              ...
              this.$nextTick(() => {
                ...
                let focusedLyric = document.querySelector(
                  '.lyric [data-lyric-set="0"].focus'
                );
                let tlfocusedLyric =
                  document.querySelector('.lyric [data-lyric-set="0"].tl') ||
                  null;
                if (focusedLyric) {
                  let sh =
                    focusedLyric.offsetTop -
                    document.querySelector("main").clientHeight / 2 +
                    focusedLyric.clientHeight / 2;
                  if (tlfocusedLyric) {
                    sh =
                      tlfocusedLyric.offsetTop -
                      document.querySelector("main").clientHeight / 2 -
      ...
      if (id) {
        url =
          ...
          `/pokaapi/lyric/?moduleName=${encodeURIComponent(
            source
          )}&id=${encodeURIComponent(id)}&time=${new Date().getTime()}`;
    ...
    getLyricByKeyword(keyword, set = true) {
      ...
      this.axios(
        _setting(`server`) +
          `/pokaapi/searchLyrics/?keyword=${encodeURIComponent(keyword)}`
      )
      ...
      try {
        ...
        if (
          ...
          window._lrc.lyrics_all[
            window._lrc.lyrics_all.length - 2
          ].timestamp += 100;
          this.lyricTranslated = true;
          window._lrc.lyrics_all = window._lrc.lyrics_all.sort(
