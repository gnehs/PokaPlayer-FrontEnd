<template>
  <div>
    <transition :name="bgSlideAnimation" mode="out-in">
      <poka-header :blurbg="true" :title="$t('nowplaying')" :bg="audio_cover" v-if="audio_queue.length > 0" :key="audio_index" />
      <poka-header v-if="!audio_queue.length > 0" key="2" />
    </transition>
    <transition name="slide-right" mode="out-in">
      <poka-parse-songs v-if="audio_queue.length > 0" :data="audio_queue" now-playing :active-index="audio_index">
      ...
      <v-card v-if="!audio_queue.length > 0" class="mx-auto blur-card" max-width="344" style="margin-top: 64px" key="card">
    ...
    <v-fab-transition>
      <v-btn color="primary" fab large dark bottom right fixed style="bottom: calc(16px + 69px)" v-if="audio_queue.length > 0" @click="audio_clean">
