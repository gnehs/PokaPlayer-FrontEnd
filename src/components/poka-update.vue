...
export default {
  ...
  created() {
    this.axios
      ...
      .then(data => {
        let isAdmin =
          JSON.parse(sessionStorage.getItem("login")).role == "admin" || false;
    ...
    async fetchNewVersion(currentVersion, isAdmin) {
      ...
      if (!githubRes) {
        githubRes = await fetch(
          "https://api.github.com/repos/gnehs/PokaPlayer/releases"
        )
          .then(e => e.json())
          .then(e => {
      ...
      if (
        this.compareVersion(currentVersion, githubRes[0].tag_name) ||
        this.debug
      ) {
